...
int thread_init(uint32_t max_threads, uint32_t stack_size,
                void (*idle_func)(void), uint32_t max_mutexes);

/**
 * @brief      Create a new thread running the given function. The thread will
 *             not be created if the UB test fails, and in that case this
 ...
 * @return     0 on success or -1 on failure
 */
int thread_create(void (*fn)(void *vargp), uint32_t prio, uint32_t C,
                  uint32_t T, void *vargp);

/**
 * @brief      Allow the kernel to start running the thread set.
 *
 *             This function should enable SysTick and thus enable your
 *             scheduler. It will not return immediately unless there is an
 *error. It may eventually return successfully if all thread functions are
 *   		   completed or killed.
 *
